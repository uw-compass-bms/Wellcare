...
import { QUOTE_PROMPT } from '@/lib/document-extraction/quote-prompt';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Call AI with quote-specific prompt
    const aiResult = await callAI(b64data, QUOTE_PROMPT, fileName);
    ...
    const errorResponse = handleAPIError(error, 'Quote');
