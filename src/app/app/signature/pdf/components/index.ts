// Main production component
export { ProductionSignatureCanvas } from './ProductionSignatureCanvas';

// Core components (can be used independently if needed)
export { PDFViewer } from './PDFViewer';
export { FieldItem } from './FieldItem';
export { FieldPalette } from './FieldPalette';
export { VirtualizedFieldLayer } from './VirtualizedFieldLayer';
export { MobileFieldPalette } from './MobileFieldPalette';

// Types
export type { Field, FieldMeta, FieldType } from './FieldItem';