...
import { BusinessRuleProps, BusinessRuleResult, RULE_STATUS_CONFIG, VehicleAgeResult, SingleVehicleAgeResult } from './types';
import { QuoteData } from '../../../types';

export default function VehicleAgeValidation({ 
  documents, 
  onResultChange, 
  ...
  useEffect(() => {
    ...
    const validateRule = async () => {
      ...
      try {
        ...
        console.log('Vehicle age validation request:', requestData);
  ...
  return (
    <div className={`border rounded-lg p-4 ${config.bgColor} ${config.borderColor}`}>
      ...
      <div className={`space-y-2 text-sm ${config.color}`}>
        ...
        {/* 多车辆结果显示 */}
        {result.result && (result.result as VehicleAgeResult).vehicles && (result.result as VehicleAgeResult).vehicles!.length > 0 ? (
          <div className="mt-3 space-y-3">
            {(result.result as VehicleAgeResult).vehicles!.map((vehicle: SingleVehicleAgeResult, index: number) => (
              <div key={index} className="text-sm">
                <div className="font-medium mb-2">{vehicle.vehicle_info || `Vehicle ${index + 1}`}</div>
                
                <div className="text-xs space-y-1">
                  <div><strong>Vehicle Year:</strong> {vehicle.vehicle_year || 'N/A'}</div>
                  <div><strong>Vehicle Age:</strong> {vehicle.vehicle_age !== null ? `${vehicle.vehicle_age} year(s)` : 'N/A'}</div>
                  {vehicle.coverage_details && (
                    <div><strong>Coverage:</strong> 
                      {vehicle.coverage_details.all_perils ? ' All Perils' : ''} 
                      {vehicle.coverage_details.comprehensive ? ' Comprehensive' : ''} 
                      {vehicle.coverage_details.collision ? ' Collision' : ''} 
                    </div>
                  )}
                  <div><strong>Status:</strong> {vehicle.recommendation}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          result.result && (
            <div className="mt-3 text-xs">
              <div><strong>Vehicle Year:</strong> {(result.result as VehicleAgeResult).vehicle_year || 'N/A'}</div>
              <div><strong>Vehicle Age:</strong> {(result.result as VehicleAgeResult).vehicle_age !== null && (result.result as VehicleAgeResult).vehicle_age !== undefined ? `${(result.result as VehicleAgeResult).vehicle_age} year(s)` : 'N/A'}</div>
              {(result.result as VehicleAgeResult).coverage_details && (
                <div><strong>Coverage:</strong> 
                  {(result.result as VehicleAgeResult).coverage_details?.all_perils ? ' All Perils' : ''} 
                  {(result.result as VehicleAgeResult).coverage_details?.comprehensive ? ' Comprehensive' : ''} 
                  {(result.result as VehicleAgeResult).coverage_details?.collision ? ' Collision' : ''} 
                </div>
              )}
            </div>
          )
